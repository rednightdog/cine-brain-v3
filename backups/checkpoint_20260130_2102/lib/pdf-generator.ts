...
export async function generateCineListPDF(data: PDFItem[], project: ProjectData | null): Promise<string> {
    ...
    return new Promise((resolve, reject) => {
        try {
            ...
            doc.text(`CINEBRAIN • CAMERA REPORT • V${project?.version || 1}`, LEFT_MARGIN, currentY, { characterSpacing: 2 });
                    ...
                    if (cat.id === 'LNS') {
                        ...
                        Object.entries(lensGroups).forEach(([seriesKey, items]) => {
                            ...
                            if (specs.length > 0) {
                                ...
                                doc.fontSize(7).font('Inter').fillColor(COLOR_GREY_LIGHT).text(specs.join(' • '), LEFT_MARGIN + titleW + 10, startY + 2);
                            }
                        ...
                        unitItems.forEach(item => {
                            ...
                            const specs: string[] = [];
                            if (item.sensor_size) specs.push(item.sensor_size.toLowerCase());
                            if (item.weight_kg) specs.push(`${item.weight_kg}kg`);
                            if (item.front_diameter_mm) specs.push(`${item.front_diameter_mm}mm`);

                            if (specs.length > 0) {
                                doc.fontSize(7).font('Inter').fillColor(COLOR_GREY_LIGHT).text(specs.join(' • '), LEFT_MARGIN + xOff, doc.y);
                            }
